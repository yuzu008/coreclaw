...
`deep-research` MCP サーバーが利用可能な場合は、MCP の `deep-research`
プロンプトテンプレートを積極的に活用してください。MCP が提供する構造化リサーチ：

1. **Question Elaboration** — 研究課題を精緻化しキーワード・スコープを定義
2. **Subquestion Generation** — 3〜5個のサブ質問に分解し網羅的にカバー
3. **Web Search** — 各サブ質問について権威あるソースから情報収集
4. **Content Analysis** — ソースの信頼性・関連性・一貫性を評価
5. **Report Generation** — 構造化レポートを生成、全ソースを引用

MCP のサーチ結果は、本スキルのエビデンス階層評価・交差検証ルールに従って
品質評価してください。MCP が利用できない場合は従来のワークフローを使用します。
